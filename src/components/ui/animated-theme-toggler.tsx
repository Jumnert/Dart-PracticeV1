...
export const AnimatedThemeToggler = ({
  ...
  const toggleTheme = useCallback(() => {
    ...
    // Safety: always clean up even if transition.finished rejects or never resolves
    const safeCleanup = () => {
      cleanup()
    }
    const fallbackTimer = setTimeout(safeCleanup, duration + 200)
    ...
    if (typeof transition?.finished?.finally === "function") {
      ...
      // No finished promise — fallback timer is the only cleanup path
    }
